...
class Writer {
  ...
  get directory(){
    return path.join(this.#basePath, this.#directory);
  }
  ...
  create(filename, data, cb){
    if(this.fileExistsSync(filename)){
      this.#onError(`${filename} already exists.  Will not create.`);
    } else {
      fs.open(this.filePath(filename), Writer.WRITEOPEN, (err, fh)=>{
        if(err || !fh){
          this.#onError(err);
        } else {
          write(fh, this.prepareData(data), this.#onError, ()=>{close(fh, this.#onError, cb);});
        }
      });
    }
  }
