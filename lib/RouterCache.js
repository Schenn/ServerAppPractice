...
module.exports = class RouterCache {
  ...
  buildCacheAsync(controllerPath, cb){
    ...
    collector.collectFromPath(this.controllerPath, (metadata)=>{

    });
    fs.readdir(this.controllerPath, (err, files) => {
      files.forEach((file) => {
        this.readControllerAsync(file);
      });
      cb(this.routes);
    });
  }
  ...
  readControllerAsync(controllerFileName){
    let controllerPath = path.join(this.controllerPath, controllerFileName);
    fs.readFile(controllerPath, 'utf8', (err, controllerText)=>{
      if(err){
        console.log(err);
      } else {
        this.extractRouteFromController(controllerText, controllerFileName.replace(".js", ""));
      }
    });
  }
