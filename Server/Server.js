...
class Server {
  ...
  handle(req, res, https = false){
    ...
    if(!validity.isValid){
      this.closeResponse(res, {code: 405, message: validity.message});
    } else {
    ...
    this[this._].router.handle(data, (status=200, payload=null, headers=[])=>{
      for(let header of headers){
        res.setHeader(header.key, header.value);
      }

      this.closeResponse(res, {code: status, message: ''}, payload);
    }, https);
  }
